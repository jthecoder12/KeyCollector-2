# Key Collector 2
## A submission to the [Brackeys Game Jam 2026.1](https://itch.io/jam/brackeys-15)

View instructions on the [https://jthecoder12.itch.io/key-collector-2](itch.io page)
<br>
<br>
<br>
<br>
**Credits:**
Key Collector 2

A submission to the Brackeys Game Jam 2026.1 (https://itch.io/jam/brackeys-15)

Game made by jthecoder12 (https://github.com/jthecoder12) / (https://jthecoder12.github.io) / (https://jthecoder12.itch.io)

Licensed under the GNU General Public License Version 3.0 (GPL 3.0)

Assets:

Coin sound "coin.wav" / "Picked Coin Echo.wav" (https://opengameart.org/content/picked-coin-echo):

Author: NenadSimic (https://opengameart.org/users/nenadsimic)

Licensed under CC-BY 3.0 (https://creativecommons.org/licenses/by/3.0)

Font (Open Sans) (https://fonts.google.com/specimen/Open+Sans) / (https://github.com/googlefonts/opensans):

Designed by Steve Matteson

Licensed under the Open Font License (OFL)

Libraries used:

.NET (https://dotnet.microsoft.com)

raylib (https://raylib.com) (Zlib License)

Raylib-cs (https://github.com/raylib-cs/raylib-cs/tree/master) (Zlib License)

GLFW (https://www.glfw.org/) (Zlib License)

TinyFileDialogs (https://sourceforge.net/projects/tinyfiledialogs) (Zlib License)

TinyDialogsNet (https://github.com/nedoxff/TinyDialogsNet) (MIT License)

ImGui (https://github.com/ocornut/imgui) (MIT License)

ImGui.NET (https://github.com/ImGuiNET/ImGui.NET) (MIT License)

rlImGui-cs (https://github.com/raylib-extras/rlImGui-cs) (Zlib License)
